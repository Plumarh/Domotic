...
//  int rc;
  ...
  if (htu21rc == 0){
    ...
    //sleepms(100);
    htu21df_ReadUserReg(fd,&UserRegVal);
    htu21df_getValue(fd,&myTemp,HTU21DF_READTEMP);
...
float htu_getHumidity(){
//  int rc;
  int fd = i2c_Open(I2CBus); //program will terminate within function if bus not present.
  float myTemp,myHum,ComRH;
  ...
  if (htu21rc == 0){
    ...
    htu21df_ReadUserReg(fd,&UserRegVal);
    htu21df_getValue(fd,&myTemp,HTU21DF_READTEMP);
    htu21df_getValue(fd,&myHum,HTU21DF_READHUM);
    ComRH = htu21df_compensatedRH(myHum,myTemp);
  }
  close (fd);
